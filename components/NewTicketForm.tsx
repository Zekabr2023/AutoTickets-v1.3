...
export const NewTicketForm: React.FC<NewTicketFormProps> = ({ empresa, onCancel, onSubmit }) => {
  ...
  const [selectedAI, setSelectedAI] = useState<string>(() => {
    ...
    return saved ? JSON.parse(saved).selectedAI || '' : '';
  });
  // State for Solicitante Name (persisted separately as it doesn't change much)
  const [solicitanteNome, setSolicitanteNome] = useState(() => {
    return localStorage.getItem('lastSolicitanteNome') || '';
  });

  const [titulo, setTitulo] = useState(() => {
    const saved = localStorage.getItem('newTicketDraft');
    return saved ? JSON.parse(saved).titulo || '' : '';
  });
  const [whatIsHappening, setWhatIsHappening] = useState(() => {
    const saved = localStorage.getItem('newTicketDraft');
    return saved ? JSON.parse(saved).whatIsHappening || '' : '';
  });
  const [whatShouldHappen, setWhatShouldHappen] = useState(() => {
    const saved = localStorage.getItem('newTicketDraft');
    return saved ? JSON.parse(saved).whatShouldHappen || '' : '';
  });
  const [urgency, setUrgency] = useState<UrgencyLevel>(() => {
    const saved = localStorage.getItem('newTicketDraft');
    return saved ? (JSON.parse(saved).urgency as UrgencyLevel) || UrgencyLevel.Medium : UrgencyLevel.Medium;
  });
  ...
  // Save Solicitante Name to localStorage whenever it changes
  useEffect(() => {
    localStorage.setItem('lastSolicitanteNome', solicitanteNome);
  }, [solicitanteNome]);
  ...
  useEffect(() => {
    ...
    localStorage.setItem('newTicketDraft', JSON.stringify(draft));
  }, [titulo, whatIsHappening, whatShouldHappen, urgency, selectedAI]);
  ...
  useEffect(() => {
    const carregarIAs = async () => {
      ...
      // Only set default if nothing is selected (from draft)
      if (iasCarregadas.length > 0 && !selectedAI) {
    ...
    carregarIAs();
  }, [empresa.id]); // Removed selectedAI from dependency to avoid overwriting draft
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await ticketService.criarTicket({
        ...
        solicitante_nome: solicitanteNome, // Sending the name
        titulo: titulo.trim(),
      ...
      <div className="bg-gray-800/80 backdrop-blur-sm rounded-xl shadow-2xl p-6 sm:p-8 border border-gray-700">
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-6">
            ...
            {/* Nome do Solicitante */}
            <div>
              <label htmlFor="solicitante" className="block text-sm font-medium text-gray-300 mb-2">
